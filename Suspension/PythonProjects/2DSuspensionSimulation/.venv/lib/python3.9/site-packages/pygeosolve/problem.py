...
from scipy.optimize import basinhopping, least_squares, minimize, root, shgo
...
class Problem:
    ...
    def constrain_line_length(self, name, length):
        ...
        """
        constrain = LineLengthConstraint(self[name], length) 
    ...
    def constrain_angle_between_lines(self, line_a, line_b, angle):
        ...
        """
        constrain = LineAngleConstraint(self[line_a], self[line_b], angle)
        self.constraints.append(constrain)
        return constrain
    ...
    def solve(self, solver="minimize", **kwargs):
        """Solve the problem.
        ...
        try:
            if(solver == "minimize"):
                solution = minimize(f, x0=self.free_values, **kwargs)
            elif(solver == "root"):
                solution = root(f, x0=self.free_values, **kwargs)
            elif(solver == "ls"):
                solution = least_squares(f, x0=self.free_values, **kwargs)
            elif(solver == "bh"):
                solution = basinhopping(f, x0=self.free_values, **kwargs)
            elif(solver == "shgo"):
                solution = shgo(f, x0=self.free_values, **kwargs)
        except:
            self._update(xpre)
            raise
