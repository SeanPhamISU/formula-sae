'''
ROLL ANGLE vs RIGHT CAMBER vs X OR Y POSITION OF UCA OR LCA INNER POINT
'''
...
og_U = U
og_L = L
offsets = np.linspace(-0.3, 0.3, 16)
...
for offset in offsets:
    ...
    camber_results_u_x.append([])
    u_x_positions.append(og_U[1] + offset)
    for roll_angle in roll_angles:
        right_suspension.solve_problem(roll_angle, 0 )
        camber_results_u_x[-1].append(right_suspension.get_camber())
    print("Data collection iteration done")
print("Data collection for Roll angle vs Camber vs UCA x position complete!")

for offset in offsets:
    update_u( (og_U[0], og_U[1] + offset), left_suspension, right_suspension )
    camber_results_u_y.append([])
    u_y_positions.append(og_U[1] + offset)
    for roll_angle in roll_angles:
        right_suspension.solve_problem(roll_angle, 0 )
        camber_results_u_y[-1].append(right_suspension.get_camber())
    print("Data collection iteration done")
print("Data collection for Roll angle vs Camber vs UCA y position complete!")

update_u( (og_U[0], og_U[1]), left_suspension, right_suspension )

for offset in offsets:
    update_l( (og_L[0] + offset, og_L[1]), left_suspension, right_suspension )
    camber_results_l_x.append([])
    l_x_positions.append(og_L[1] + offset)
    for roll_angle in roll_angles:
        right_suspension.solve_problem(roll_angle, 0 )
        camber_results_l_x[-1].append(right_suspension.get_camber())
    print("Data collection iteration done")
print("Data collection for Roll angle vs Camber vs LCA x position complete!")

for offset in offsets:
    update_l( (og_L[0], og_L[1] + offset), left_suspension, right_suspension )
    camber_results_l_y.append([])
    l_y_positions.append(og_L[1] + offset)
    for roll_angle in roll_angles:
        right_suspension.solve_problem(roll_angle, 0 )
        camber_results_l_y[-1].append(right_suspension.get_camber())
    print("Data collection iteration done")
print("Data collection for Roll angle vs Camber vs LCA y position complete!")
...
axs[0][0].set_zlabel('Camber Angle (deg)')
axs[0][0].set_title('UCA X Position vs. Roll & Heave')

X, Y = np.meshgrid(roll_angles, u_y_positions)
Z = np.array(camber_results_u_y)
...
axs[0][1].set_zlabel('Camber Angle (deg)')
axs[0][1].set_title('UCA Y Position vs. Roll & Heave')

X, Y = np.meshgrid(roll_angles, l_x_positions)
Z = np.array(camber_results_l_x)
...
axs[1][0].set_zlabel('Camber Angle (deg)')
axs[1][0].set_title('LCA X Position vs. Roll & Heave')

X, Y = np.meshgrid(roll_angles, l_y_positions)
Z = np.array(camber_results_l_y)
...
axs[1][1].set_zlabel('Camber Angle (deg)')
axs[1][1].set_title('LCA Y Position vs. Roll & Heave')
